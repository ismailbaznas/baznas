    ...
    <div className="flex flex-col h-full bg-[#004229] dark:bg-[#041d0a] text-white p-5 select-none font-jakarta">
      ...
      <div className="flex items-center justify-between pb-6 mb-4 border-b border-white/10">
        ...
        {onClose && (
          <button 
            onClick={onClose} 
            className="md:hidden text-white/70 hover:text-white w-10 h-10 min-w-[40px] min-h-[40px] flex items-center justify-center rounded-xl hover:bg-white/10 active:scale-95 transition-all cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              onClick={onClose}
              className={cn(
                "flex items-center gap-3 px-3.5 py-2.5 min-h-[40px] rounded-xl text-xs font-semibold transition-all duration-150 active:scale-[0.98] group",
                active
      ...
      <div className="mt-auto pt-4 border-t border-white/10 space-y-3">
        <Link 
          href="/admin/profile" 
          onClick={onClose}
          className="p-3 rounded-xl bg-white/5 hover:bg-white/10 active:scale-[0.98] transition-all border border-white/10 flex items-center gap-3 group"
        ...
        <button
          ...
          disabled={loading}
          className="w-full min-h-[40px] bg-red-500/10 hover:bg-red-500/20 active:scale-[0.98] text-red-300 hover:text-red-200 border border-red-500/30 py-2.5 px-3 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-400"
        >
...
export default function AdminLayoutClient({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-[100dvh] bg-[#fbf9f4] dark:bg-[#051808] font-jakarta antialiased selection:bg-[#075C3B] selection:text-white">
      {/* Sidebar */}
        ...
        <header className="sticky top-0 z-30 h-16 bg-white/90 dark:bg-[#08240e]/90 backdrop-blur border-b border-surface-variant/40 dark:border-[#0f3d17] px-4 sm:px-8 flex items-center justify-between transition-colors">
          <div className="flex items-center gap-3">
            <button
              ...
              className="md:hidden w-10 h-10 min-w-[40px] min-h-[40px] rounded-xl flex items-center justify-center text-on-surface-variant hover:bg-slate-100 dark:hover:bg-zinc-800 active:scale-95 transition-all cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#075C3B] dark:focus-visible:ring-[#8cd6ac]"
